...
\keyword{대규모 언어모델, 환각 탐지, Semantic Entropy, Semantic Energy, Zero-SE 문제, Adaptive Threshold}
\begin{abstract}
...
TruthfulQA 데이터셋(K=5)에서의 실험 결과:
\begin{itemize}
    \item Zero-SE 영역이 전체의 19\%를 차지하며, 이 중 73.7\%가 환각
    \item Zero-SE 영역에서 Energy AUROC 0.736 달성 (SE는 판별 불가)
    \item SE-gated cascade는 SE 단독 대비 AUROC +0.030 개선 (0.613 → 0.643)
    \item 합집합 탐지율 89.6\% 달성
\end{itemize}
...
환각 탐지 함수 $D$를 다음과 같이 정의하자:
\begin{equation}
    D(q) = \begin{cases}
        Energy(q) & \text{if } SE(q) < \tau \\
        SE(q) & \text{if } SE(q) \geq \tau
    \end{cases}
\end{equation}

이 cascade 함수는 SE가 낮은 영역(Zero-SE)에서 Energy를, SE가 높은 영역에서 SE를 사용한다.
...
\section{Adaptive Cascade: 입력 기반 동적 가중치}

고정 임계값 $\tau$의 한계는 cross-dataset 환경에서 일반화가 어렵다는 것이다. 실험 결과(exp06), 학습 기반 가중치 예측 모델은 같은 분포 내에서는 AUROC 0.91을 달성하지만, 새로운 데이터셋에서는 baseline보다 성능이 떨어졌다.

이를 해결하기 위해, 각 샘플의 특성을 보고 가중치를 결정하는 \textbf{Adaptive Cascade}를 제안한다:

\begin{equation}
    Score = w(x) \cdot Energy + (1 - w(x)) \cdot SE
\end{equation}

여기서 $w(x)$는 입력 $x$의 특성에 따라 결정되는 가중치 함수이다.

\subsection{Rule-based Adaptive}

학습 없이 해석 가능한 규칙으로 가중치를 결정한다:

\begin{equation}
    w(x) = \begin{cases}
        0.9 & \text{if } SE(x) < 0.1 \text{ (Zero-SE)} \\
        0.1 & \text{if } SE(x) > 0.5 \text{ (High-SE)} \\
        \text{linear interpolation} & \text{otherwise}
    \end{cases}
\end{equation}

\subsection{Cluster-aware Adaptive}

클러스터 수를 직접 활용한다. Zero-SE의 정의가 ``단일 클러스터''이므로:

\begin{equation}
    w(x) = \begin{cases}
        0.9 & \text{if } |C| = 1 \text{ (단일 클러스터)} \\
        0.5 & \text{if } |C| = 2 \\
        0.1 & \text{if } |C| \geq 3
    \end{cases}
\end{equation}

\subsection{Hybrid Adaptive}

SE 값과 클러스터 수를 모두 고려하는 복합 방법:

\begin{enumerate}
    \item 클러스터 1개 → Energy 강하게 선호 ($w = 0.9$)
    \item 클러스터 2개 이상 + SE 낮음 → Energy 약하게 선호 ($w = 0.7$)
    \item 클러스터 2개 이상 + SE 높음 → SE 선호 ($w = 0.1$)
\end{enumerate}

이 방법의 장점은:
\begin{itemize}
    \item \textbf{학습 불필요}: Cross-dataset 일반화 가능
    \item \textbf{해석 가능}: 왜 그 가중치를 선택했는지 설명 가능
    \item \textbf{샘플별 최적화}: 각 입력에 맞는 메트릭 선택
\end{itemize}
...
TruthfulQA 데이터셋을 사용하였다. 이 데이터셋은 인간이 흔히 가지는 오개념을 유도하는 질문들로 구성되어 있어, LLM의 환각 탐지 연구에 적합하다.
...
\begin{figure}[htbp]
    ...
    \caption{SE 구간별 SE vs Energy AUROC 비교: Crossover 패턴 확인}
    \label{fig:crossover}
\end{figure}
...
Very High (1.0+) & 95 & 85.3\% & \textbf{0.658} & 0.422 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
    ...
    \caption{SE와 Energy의 환각 탐지 영역 비교 (80th percentile 기준)}
...
Energy만 탐지하는 17.7\%는 SE로는 절대 잡을 수 없는 환각이다.
...
\begin{enumerate}
    ...
    \item \textbf{상보성}: SE와 Energy의 합집합 탐지율은 89.6\%에 도달한다.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Adaptive Threshold}: 입력에 따라 $\tau$를 동적으로 결정하는 방법 개발. 현재 고정 임계값은 cross-dataset 환경에서 한계가 있다.
    
    \item \textbf{다양한 데이터셋 검증}: TriviaQA, NaturalQuestions, HaluEval 등에서 검증 필요.
    ...
    \item \textbf{KESFE/AHSFE 확장}: 문헌조사에서 제안된 Kernel 기반 또는 Adaptive 가중치 방법 적용.
\end{enumerate}
...
\begin{lstlisting}[caption={프로젝트 구조}]
hallucination_lfe/
├── packages/
│   └── hfe-core/          # 핵심 라이브러리
│       └── src/hfe_core/
...
\begin{lstlisting}[caption={SemanticEntropyCalculator}]
...
\begin{lstlisting}[caption={SemanticEnergyCalculator}]
...
\begin{lstlisting}[caption={AdaptiveCascade (입력 기반 동적 가중치)}]
class AdaptiveCascade:
    def compute(self, se, energy, num_clusters):
        # Hybrid: 클러스터 수와 SE 모두 고려
        if num_clusters == 1:
            w_energy = 0.9  # 단일 클러스터 -> Energy
        elif se < 0.1:
            w_energy = 0.7  # 낮은 SE -> Energy 선호
        elif se > 0.5:
            w_energy = 0.1  # 높은 SE -> SE 선호
        else:
            w_energy = 0.7 - 0.6 * (se - 0.1) / 0.4
        
        w_se = 1 - w_energy
        score = w_energy * energy + w_se * se
        return score, w_energy, w_se
\end{lstlisting}
